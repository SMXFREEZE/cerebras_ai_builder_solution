...
`demo:loom` records a fresh live browser walkthrough with Playwright, generates narration, then renders a normal-speed MP4. It covers the official Loom points plus every major app surface: receive, duplicate receive, serial conflict recovery, store, deploy validation, deploy writebacks, transfer, store-from-in-service de-racking, manager filtering, asset detail, reconciliation, and barcode coverage.
